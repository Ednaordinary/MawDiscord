    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if int(history[-1].message_id) == interaction.message.id:
            try:
                ...
                pass
            await interaction.response.edit_message(
                content=self.content.value, view=self.view
            )
            global model_queue
            model_queue.append(
                CharacterGen(
                    character_message=interaction.message,
                    local_character=self.character,
                    thread=interaction.channel,
                    user_message=None,
                    vc=False,
                    start=self.content.value,
                )
            )
        else:
...
class RedoMessageButton(discord.ui.View):
    ...
    @discord.ui.button(
        label="Redo", style=discord.ButtonStyle.primary
    )  # , custom_id="maw-redo"
...
class EditAndRedoMessageButton(discord.ui.View):
    ...
    async def edit_button(
        ...
        if config.locked_id != 0 and config.locked_id != interaction.user.id:
            ...
            await interaction.response.send_modal(
                EditMessageModal(
                    interaction.message.content, local_character, self.thinks
                )
            )
    ...
    @discord.ui.button(
        label="Redo w/ Start", style=discord.ButtonStyle.primary
    )  # , custom_id="redo-start-edit-and-redo-message"
    async def redo_start_button(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ):
        config = read_config(
            "./characters/"
        ...
        if config.locked_id != 0 and config.locked_id != interaction.user.id:
            ...
            for child in self.children:
                try:
                    ...
                    pass
            await interaction.response.send_modal(
                RedoStartMessageModal(
                    interaction.message.content[:1000], local_character, self
                )
            )
...
class EditMessageButton(discord.ui.View):
    ...
    @discord.ui.button(
        label="Delete", style=discord.ButtonStyle.red, custom_id="delete-edit-message"
    )
    async def delete_button(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ):
...
class VoiceResponse(discord.ui.View):
    ...
    async def transcribe_button(
        ...
        await interaction.response.edit_message(
            view=VoiceTranscribe(session=self.session)
        )

    @discord.ui.button(label="Opt")
    async def opt_button(
        self, button: discord.ui.Button, interaction: discord.Interaction
    ):
        global voice_opt
...
class VoiceTranscribe(discord.ui.View):
    ...
    async def response_button(
        ...
        await interaction.response.edit_message(
            view=VoiceResponse(session=self.session)
        )
...
def history_to_llama_auto(history, config, start, cutoff, maw):
    ...
    system_prompt = tokenizer.apply_chat_template(
        conversation=[
            {"role": "system", "content": config.system_prompt.replace(r"\n", "\n")}
        ],
        tokenize=True,
        return_tensors="pt",
    ...
    if start != "":
        start_message = tokenizer.encode(
    ...
    for idx, message in enumerate(history):
        ...
        if idx != 0 and grouped_messages[-1].role == role:
            ...
            if len(history) > idx + 1 and history[idx + 1].role == role:
                ...
                grouped_messages.append(
                    GroupedMessage(message.content.replace(r"\n", "\n"), role)
                )
    ...
    if config.environment_prompt.strip() != "":
        environment_prompt = tokenizer.apply_chat_template(
            conversation=[
                {
                    "role": "system",
                    "content": config.environment_prompt.replace(r"\n", "\n"),
                }
            ],
            tokenize=True,
            return_tensors="pt",
        ...
        if token_length + environment_prompt.shape[1] < (
            cutoff - system_prompt.shape[1]
        ):
    ...
    decoded = tokenizer.batch_decode(llama, skip_special_tokens=False)[0].replace(
        r"\n", "\n"
    )
    print(decoded)
    llama = tokenizer.encode(decoded, add_special_tokens=False, return_tensors="pt")
    # llama = "".join(llama)
        ...
        if idx != 0 and grouped_messages[-1].role == role:
            ...
            if len(history) > idx + 1 and history[idx + 1].role == role:
                grouped_messages.append(
                    GroupedMessage(
                        (
                            "-----\n\nThe above is past history. This is the prompt a user is now asking: "
                            if maw
                            else ""
                        )
                        + message.content.replace(r"\n", "\n"),
    ...
    for idx, message in enumerate(grouped_messages):
        ...
        if token_length + llama_message.shape[1] < (cutoff - system_prompt.shape[1]):
            ...
            llama.append(
                torch.tensor(
                    llama_message[0][
                        int((cutoff - system_prompt.shape[1]) - (token_length)) * -1 :
                    ]
                ).unsqueeze(0)
            )
    ...
    try:
        ...
        if "".join(thinks).strip() != "":
            await hook.edit_message(
                ...
                thread=thread,
                view=EditAndRedoMessageButtonThink(thinks=thinks),
            )
            thought_retainer[message.id] = thinks
        else:
            await hook.edit_message(
...
async def edit_add_hookeditbutton(hook, message, content, thread, thinks):
    ...
    if "".join(thinks).strip() != "":
        ...
        await hook.edit_message(
            ...
            message_id=message.id,
            thread=thread,
            view=EditMessageButtonThink(thinks=thinks),
        )
        thought_retainer[message.id] = thinks
    else:
        await hook.edit_message(
...
def global_model_user_update(message):
    ...
    for request in modelusers:
        if isinstance(request, CharacterInferRequest) or isinstance(
            request, VCInferRequest
        ):
            if not request.thread:
                asyncio.run_coroutine_threadsafe(
                    coro=request.character_message.edit(message), loop=client.loop
                )
            else:
                asyncio.run_coroutine_threadsafe(
                    coro=hook_list[
                        request.character_message.channel.parent.id
                    ].edit_message(
...
async def async_watcher():
    ...
    while True:
        if model_queue == []:
            ...
            typing_channels.append(
                [channel if not thread else thread, time.time() - 10]
            )
            if not model_loaded:
                ...
                def model_load_callback(current, total):
                    ...
                    if (
                        time.time() > model_callback_limiter + 1.0
                    ):  # only bursts for the first second or so, so its rate can be higher
                        if current_gen.character.maw:
                            asyncio.run_coroutine_threadsafe(
                                coro=current_gen.character_message.edit(
                                    str(int(current * 100 / total)) + "%"
                                ),
                                loop=client.loop,
                            )
                        else:
                            asyncio.run_coroutine_threadsafe(
                                coro=temp_edit(
                                    current_gen.character_message.id,
                                    thread,
                                    str(int(current * 100 / total)) + "%",
                                    channel.id,
                                ),
                ...
                cache = ExLlamaV2Cache_Q4(
                    model, lazy=True, max_seq_len=cache_size * 256
                )
                model.load_autosplit(
                    cache, progress=False, callback=model_load_callback
                )
            ...
            if current_gen.auto:
                model_input, local_token_count = history_to_llama_auto(
                    history,
                    current_gen.character.config,
                    current_gen.start,
                    ((cache_size - 10) * 256),
                    current_gen.character.maw,
                )
            else:
                model_input, local_token_count = history_to_llama(
                    history,
                    current_gen.character.config,
                    current_gen.start,
                    (cache_size - 10) * 256,
                    ...
                    if result["stage"] == "streaming":
                        ...
                        if vc_session:
                            ...
                            if "<｜end▁of▁sentence｜>" in vc_response:
                                vc_response = vc_response.replace(
                                    "<｜end▁of▁sentence｜>", ""
                                )
                        ...
                        if (
                            "<-" in response
                            and "<think>" not in final_response
                            and character.maw
                        ...
                        if (
                            ...
                            for word in [
                                x for x in final_response.split(" ") if x != None
                            ]:
                                end_ids = [".", "\n", "!", "?", "*"]
                                for end_id in end_ids:
                                    word.replace(end_id, "")
                                if word.lower().strip() in [
                                    (x.global_name if x.global_name else x.name)
                                    .lower()
                                    .strip()
                                    for x in message.guild.humans
                                ]:
                                    mention = message.guild.humans[
                                        [
                                            (x.global_name if x.global_name else x.name)
                                            .lower()
                                            .strip()
                                            for x in message.guild.humans
                                        ].index(word.lower().strip())
                                    ].mention
                        ...
                        if time.time() - limiter > 1.2:
                            ...
                            if character.maw:
                                asyncio.run_coroutine_threadsafe(
                                    coro=message.edit(
                                        final_response.replace("<think>", "").replace(
                                            "</think>", ""
                                        )[:1999]
                                    ),
                                    loop=client.loop,
            ...
            if character.maw:
                if (
                    ...
                    in [x.character_message.channel.id for x in model_queue[1:]]
                    and not current_gen.vc
                ):
                    asyncio.run_coroutine_threadsafe(
                        coro=edit_add_redobutton(
                            message,
                            final_response.replace("<think>", "").replace(
                                "</think>", ""
                            )[:1999],
                    ...
                    if thinks != []:
                        asyncio.run_coroutine_threadsafe(
                            coro=message.edit(
                                ...
                                view=ThinkButton(thinks=thinks),
                            ),
                            loop=client.loop,
                        )
                    else:
            ...
            if current_gen.character.maw:
                ...
                history.append(
                    MawCharacterMessage(
                        re.sub("<think>.*?</think>", "", response, flags=re.DOTALL),
                        current_gen.character_message.id,
                        "character",
                    )
                )
    ...
    try:
        ...
        async for data in proto.listen():
            ...
            opus_frame = OpusFrame(
                sequence, timestamp, time.perf_counter(), ssrc, decrypted
            )
    ...
    if not speech_model:
        if not speechloading:
            ...
            try:
                ...
                speech_model = outetts.InterfaceEXL2(
                    model_version="0.2", cfg=model_config
                )
            except Exception as e:
                exc_type, exc_obj, exc_tb = sys.exc_info()
                fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def request_speech(text):
    ...
    try:
        ...
        text = text
        wav = speech_model.generate(
            text=text, temperature=0.1, repetition_penalty=1.1, speaker=speaker
        )
...
def request_whisper_text(audio):
    ...
    try:
        transcribed_text = whisper_model.transcribe(
            audio, fp16=True, hallucination_silence_threshold=2
        )
    except Exception as e:
        exc_type, exc_obj, exc_tb = sys.exc_info()
        fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
def user_listener(session, user, proto):
    try:
        ...
        asyncio.run_coroutine_threadsafe(
            coro=session.thread.send(
                "Finished calibrating audio for " + str(user.name)
            ),
            loop=client.loop,
        )
        while proto.is_connected():
            ...
            try:
                ...
                if not dq.empty():
                    ...
                    if (
                        len(audio_data) > 1500
                    ):  # only transcribe if there is something to transcribe
                        dq.queue.clear()
                    ...
                    if (
                        transcript_joined != ""
                        and transcript_joined != "you"
                        and not re.fullmatch(r"^[. ]*$", transcript_joined)
                    ):
                        hook = asyncio.run_coroutine_threadsafe(
                            coro=get_webhook(session.thread.parent), loop=client.loop
                        ).result()
                        if not exclusive[session]:
                            print("Session isn't exclusive")
                            if (
                                "." in transcript_joined
                                or "?" in transcript_joined
                                or "!" in transcript_joined
                            ):
                                ...
                                maw_message = asyncio.run_coroutine_threadsafe(
                                    coro=session.thread.send("..."), loop=client.loop
                                ).result()
                                if user_hook_message != None and maw_message != None:
                                    ...
                                    if os.path.isdir(relative_path):
                                        ...
                                        config.system_prompt = (
                                            ...
                                            + session.proto.channel.name
                                            + ". Since you are in a voice channel, responses should be short, about 1-3 sentences long."
                                        )
                                    ...
                                    user_message = MawCharacterMessage(  # "The user " + str(user.display_name) + " said: " +
                                        content=transcript_joined,
                                        message_id=str(user_hook_message.id),
                                        role="user",
                                    )
                                    model_queue.append(
                                        CharacterGen(
        ...
        if voice_play[session] != []:
            try:
                ...
                source = discord.FFmpegPCMAudio(
                    sourcebytes, pipe=True, options="-vn -filter:a 'volume=1.5'"
                )
                try:
                    proto.play(source)
                except Exception as e:
        ...
        for user_id in voice_data[session].keys():
            if user_id not in user_threads.keys():
                ...
                user_threads[user_id] = threading.Thread(
                    target=user_listener, args=[session, user, proto]
                )
                    ...
                    try:
                        ...
                        if len(play_audio) != 0:
                            with io.BytesIO() as play_bytes:
                                ...
                                play_audio = np.concatenate(
                                    (empty_torch, play_audio, empty_torch)
                                )
                                ...
                                scipy.io.wavfile.write(
                                    play_bytes, rate=24000, data=play_audio
                                )
                                # , encoding='PCM_S'
                                # , bits_per_sample=16
                                play_bytes.seek(0)
        ...
        for idx, i in enumerate(typing_channels):
            if i[1] < time.time() - 8:
                ...
                if isinstance(i[0], discord.StageChannel):
                    continue
                asyncio.run_coroutine_threadsafe(
                    coro=i[0].trigger_typing(), loop=client.loop
                )
...
def auto_watcher():
    ...
    while len(maw_auto_servers) > 0:
        ...
        if (
            server in maw_auto_watched.keys()
            and len(list(maw_auto_watched[server].keys())) > 0
        ):
            user = random.choice(list(maw_auto_watched[server].keys()))
            print("Chose user", user, "server", server)
            if (
                maw_auto_watched[server][user][0] < (time.time() - (60 * 60 * 10))
            ) and maw_auto_sample[server] == True:
                ...
                if channel != None:
                    ...
                    if os.path.isdir(relative_path):
                        if random.randint(1, 3) == 1:  # 1 in 3 chance to respond
                            ...
                            character = MawCharacter("Maw", config, True)
                            maw_message = asyncio.run_coroutine_threadsafe(
                                coro=channel.send(content="..."), loop=client.loop
                            ).result()
                            model_queue.append(
                                CharacterGen(
                                    character_message=maw_message,
                                    ...
                                    user_message=None,
                                    vc=False,
                                    start="<think>\n\nTo respond as Maw in a way that would build on the conversation, I should",
                                    auto=user,
                                )
                            )
...
async def on_message(message):
    ...
    if isinstance(message.channel, discord.TextChannel):
        if (
            ...
            or "<@" + str(client.user.id) + ">" in message.content.lower()
            or (
                message.reference != None
                and message.reference.cached_message != None
                and message.reference.cached_message.author.id == client.user.id
            )
    ...
    if (
        not maw_response
        and not character_response
        and not message.author.id == client.user.id
        and message.guild
    ):
                ...
                if message.content != "":
                    if message.guild.id in maw_auto_servers:
                        ...
                        try:
                            ...
                            maw_auto_watched[message.guild.id] = {
                                message.author: [time.time(), message.channel]
                            }
                        else:
                            maw_auto_watched[message.guild.id][message.author] = [
                                time.time(),
                                message.channel,
                            ]
                    maw_auto_watched
                    relative_server_path = "./servers/" + str(message.guild.id)
                    relative_thread_path = (
                        "./characters/"
                        + str(message.guild.id)
                        + "/"
                        + str(message.channel.id)
...
async def on_voice_state_update(member, before, after):
    ...
    voice_file.close()
    watched = [
        int(i.split("-")[0][:-1]) if i[:-1] == "\n" else int(i.split("-")[0])
        for i in voice_file_lines
    ]
    if after.channel.id in watched:
        ...
        if after.channel.members != []:
            global maw_voice_channels
            if (
                not member.guild in [x.guild for x in maw_voice_channels]
                and text_channel != None
            ):
                try:
                    ...
                    threading.Thread(
                        target=await_voice_allocation, args=[session]
                    ).start()
                except Exception as e:
                    exc_type, exc_obj, exc_tb = sys.exc_info()
                    fname = os.path.split(exc_tb.tb_frame.f_code.co_filename)[1]
...
async def async_await_voice_allocation(session):
    ...
    if alloc_message != None:
        alloc_message = asyncio.run_coroutine_threadsafe(
            coro=alloc_message.edit("Loading models..."), loop=client.loop
        ).result()
        ...
        try:
            ...
            try:
                await interaction.guild.change_voice_state(
                    channel=channel, self_mute=False
                )
            except Exception as e:
            ...
            print(repr(e))
            await interaction.response.send_message(
                "Failed to connect to that channel!"
            )
    else:
        await interaction.response.send_message(
            "Already connected to a channel in this server!"
        )


@voice.subcommand(
    description="Enlist a voice channel to auto-start transcription sessions.",
    name="enlist",
)
        ...
        try:
            ...
            for voice_channel in voice_file_lines:
                if voice_channel.split("-")[0] == str(channel.id):
                    await interaction.response.send_message(
                        "This channel is already enlisted for auto-transcriptions!"
                    )
                    return
            ...
            voice_file.close()
            await interaction.response.send_message(
                "Enlisted this channel to auto-transcriptions."
            )
        except:
...
@voice.subcommand(
    description="Delist a voice channel from automatic transcriptions", name="delist"
)
...
def update_watched_channels():
    ...
    for each_channel in channel_file_lines:
        channel_list.append(
            int(each_channel if each_channel[-1] != "\n" else each_channel[:-1])
        )
...
@channel_command.subcommand(
    description="Enlist a channel to auto-history.", name="enlist"
)
    ...
    try:
        ...
        for each_channel in channel_file_lines:
            if each_channel.split("-")[0] == str(channel.id):
                await interaction.response.send_message(
                    "This channel is already enlisted for auto-history!"
                )
                return
        ...
        channel_file.close()
        await interaction.response.send_message(
            "Enlisted this channel to auto-history."
        )
...
@channel_command.subcommand(
    description="Enlist a channel to auto-history.", name="enlist_all"
)
async def channel_enlist_all(
    ...
    channel_file.close()
    for channel in [
        x for x in interaction.guild.channels if isinstance(x, discord.TextChannel)
    ]:
        try:
            enlisted = False
            for each_channel in channel_file_lines:
...
@channel_command.subcommand(
    description="Delist a channel from auto-history.", name="delist"
)
...
async def token_count(
    ...
    relative_thread_path = (
        "./characters/" + str(interaction.guild.id) + "/" + str(interaction.channel.id)
    )
    if os.path.isdir(relative_thread_path):
        config = read_config(
            "./characters/"
        ...
        history = character.read_history()
        model_input, local_token_count = history_to_llama(
            history, config, "", math.pow(10, 100), character.maw
        )
        await interaction.response.send_message(
            str(local_token_count) + " tokens.", ephemeral=True
        )
    elif os.path.isdir(relative_server_path):
        config = read_config("./servers/" + str(interaction.guild.id))
        character = MawCharacter("Maw", config, True)
        ...
        await interaction.response.send_message(
            str(local_token_count) + " tokens.", ephemeral=False
        )
    else:
