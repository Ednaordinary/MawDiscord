...
class CharacterInferRequest:
    ...
    async def infer(self):
        ...
        model_input, local_token_count = history_to_llama(
            history, self.character.config, self.start, 126000, self.character.maw
        )
        ...
        async def run_job(idx):
            ...
            async for result in job:
                ...
                if self.character.maw and not isinstance(
                    self.character_message.channel, discord.DMChannel
                ):
                    ...
                    for image in re.findall(find_image, completion):
                        ...
                        if image != "":
                            with open("../DanteMode/queue.txt", "a") as image_queue:
                                image_queue.write(
                                    "\n"
                                    + str(self.character_message.channel.id)
                                    + "|"
                                    + str(image).replace("\n", "\\n")
                                )
                ...
                if time.time() - limiter > 1.0 and idx == 0:
                    ...
                    if character.maw:
                        asyncio.run_coroutine_threadsafe(
                            coro=self.character_message.edit(completion[:1999]),
                            loop=client.loop,
                        )
                    else:
                        asyncio.run_coroutine_threadsafe(
                            coro=hook_list[
                                self.character_message.channel.parent.id
                            ].edit_message(
        ...
        if self.character.maw:
            ...
            history.append(
                MawCharacterMessage(all_out[0], self.character_message.id, "character")
            )
        gc.collect()
...
class EditAndRedoMessageButton(discord.ui.View):
    ...
    async def edit_button(
        ...
        if config.locked_id != 0 and config.locked_id != interaction.user.id:
            ...
            await interaction.response.send_modal(
                EditMessageModal(interaction.message.content, local_character)
            )
        ...
        if config.locked_id != 0 and config.locked_id != interaction.user.id:
            ...
            await interaction.response.send_modal(
                RedoStartMessageModal(
                    interaction.message.content, local_character, self
                )
            )
    ...
    while True:
        if model == None and modelusers != []:
            ...
            def model_load_callback(current, total):
                ...
                if (
                    time.time() > model_callback_limiter + 0.6
                ...
                try:
                    ...
                    if cache_amount < 20:
                        ...
                        cache = ExLlamaV2Cache(
                            model, lazy=True, max_seq_len=cache_amount * 256
                        )
                else:
                    break
...
async def async_watcher():
    ...
    while True:
        if model_queue == []:
            ...
            sampler = ExLlamaV2Sampler.Settings(
                top_p=0.95, top_k=100, min_p=0.01, temperature=temp
            )
            banned_strings = [
                "I cannot provide",
                "I cannot assist",
                ...
                if results != []:
                    ...
                    if result["stage"] == "streaming":
                        ...
                        if time.time() - limiter > 1.2:
                            ...
                            if character.maw:
                                asyncio.run_coroutine_threadsafe(
                                    coro=message.edit(final_response[:1999]),
                                    loop=client.loop,
                                )
                            else:
                                asyncio.run_coroutine_threadsafe(
            ...
            if character.maw:
                if (
                    ...
                    asyncio.run_coroutine_threadsafe(
                        coro=edit_add_redobutton(message, final_response[:1999]),
                        loop=client.loop,
                    )
                else:
                    asyncio.run_coroutine_threadsafe(
                        coro=message.edit(final_response[:1999]), loop=client.loop
                    )
            else:
            ...
            if current_gen.character.maw:
                ...
                history.append(
                    MawCharacterMessage(
                        response, current_gen.character_message.id, "character"
                    )
                )
