...
model_args = dict(
    max_new_tokens=768, use_cache=True, do_sample=True, repetition_penalty=1.2
)  # , max_matching_ngram_size=2, prompt_lookup_num_tokens=20
        ...
        if time.time() - limiter > 0.8:
            ...
            if character.maw:
                asyncio.run_coroutine_threadsafe(
                    coro=message.edit(full_text), loop=client.loop
                )
            else:
                asyncio.run_coroutine_threadsafe(
                    coro=temp_edit(message.id, thread, full_text, channel.id),
                    loop=client.loop,
                )
    global model_queue
    if character.maw:
        if not message.channel.id in [
            ...
            asyncio.run_coroutine_threadsafe(
                coro=edit_add_redobutton(message, full_text), loop=client.loop
            )
        else:
            asyncio.run_coroutine_threadsafe(
                coro=message.edit(full_text), loop=client.loop
        ...
        if not thread.id in [x.character_message.channel.id for x in model_queue[1:]]:
            asyncio.run_coroutine_threadsafe(
                coro=edit_add_hookredobutton(
                    hook_list[channel.id], message, full_text, thread
                ),
                loop=client.loop,
            )
        else:
...
async def async_watcher():
    ...
    while True:
        if model_queue == []:
            ...
            model_input, token_length = history_to_llama(
                history, tokenizer, current_gen.character.config
            )
            ...
            streamer_thread = threading.Thread(
                target=message_updater,
                args=[
                    current_gen.character_message,
                    streamer,
                    current_gen.character,
                    thread,
                    channel,
                ],
            )
            ...
            asyncio.run_coroutine_threadsafe(
                coro=client.change_presence(
                    activity=discord.Activity(
                        ...
                        + str(
                            round(
                                len(response[0][model_input.shape[1] :])
            ...
            decoded_response = tokenizer.decode(
                response[0][model_input.shape[1] :], skip_special_tokens=True
            )
            if current_gen.character.maw:
                history.append(
                    MawCharacterMessage(
                ...
                history.append(
                    MawCharacterMessage(
                        decoded_response, current_gen.character_message.id, "character"
                    )
                )
