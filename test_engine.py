...
engine = Exl2Engine("dr1-1.5b-exl2-4.0bpw-hb8", 1024 * 400, ExLlamaV2Cache_Q4, loop, None, 512, None)
...
def run_thread(prompt):
    ...
    for i in engine.generate(prompt, add_bos=True, stop_token="<｜end▁of▁sentence｜>", max_tokens=128, sampler=sampler):
        global count
        count += 1
