...
class MawClient(discord.Client):
...
def init():
    ...
    client = MawClient(intents=intents)
...
def get_hook(channel, hooks, loop, client_id):
    try:
        ...
        all_hooks = asyncio.run_coroutine_threadsafe(
            coro=channel.webhooks(), loop=loop
        ).result(timeout=2)
        ...
        hook = asyncio.run_coroutine_threadsafe(
            coro=channel.create_webhook(name="Character hook"), loop=loop
        ).result(timeout=3)
...
def get_history(path, histories, config, char=False):
    try:
        ...
        if config != None:
            history.sys = (
                config if (char or config == None) else update_sys(config, char)
            )
            history.renew_sys()
    except:
        history = JsonHistory(
            path, config if (char or config == None) else update_sys(config, char)
        )
...
def make_status(tokens, run_time, requests):
    if run_time > 0:
        ...
        activity = discord.Activity(
            type=discord.ActivityType.watching,
            name="with " + str(requests) + " requests handled",
        )
    return activity
