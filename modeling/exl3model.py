...
class Exl3Engine:
    def __init__(self, model_id, cache_size, quant, loop, callback, draft=None):
        ...
        asyncio.run_coroutine_threadsafe(
            self._get_gen(), loop=self.engineloop.loop
        ).result()
            ...
            async for result in job:
                ...
                if not think_switch and pre_think >= max_tokens - 1024:
                    ...
                    new_ids = self.tokenizer.encode(
                        pre_think_text
                        + ". It seems I've been cut off. It's time to respond, even if I'm not quite done. I'll do my best to still get the right answer</think>",
                        add_bos=False,
                    )
                    new_ids = (
                        new_ids
                        if isinstance(new_ids, torch.Tensor)
                        else new_ids["input_ids"]
                    )
    ...
    async def _kickstart(self, prompt):
        job = AsyncJob(
            self.generator,
            max_new_tokens=100,
            input_ids=self.tokenizer.encode(prompt, add_bos=True).to("cpu")
            if isinstance(prompt, str)
            else prompt.to("cpu"),
            seed=random.randint(1, 10000000),
        )
    ...
    def end(self):
        asyncio.run_coroutine_threadsafe(
            self._exit(), loop=self.engineloop.loop
        ).result()
        self.model.unload()
        del self.generator
        del self.model
