...
class Tokenizer:
    def __init__(self, model_id, config):
        ...
        self.exl_tokenizer: ExlTokenizer = ExlTokenizer.from_config(Config.from_directory(config))
    ...
    def history_to_tokens(self, history, cutoff=None, keep_sys=True, embeds=None):
        ...
        if cutoff == None:
            ...
            return self.exl_tokenizer.encode(formed, encode_special_tokens=True, embeddings=embeds)
        else:
            if keep_sys:
                print(history)
                print(type(history))
            ...
            for i in history[::-1]:
                ...
                formed = self.tokenizer.apply_chat_template(
                    conversation=sys + fake_history[::-1], tokenize=False, add_generation_prompt=True,
                )
                print(formed)
                tokenized = self.exl_tokenizer.encode(formed, encode_special_tokens=True, embeddings=embeds)
