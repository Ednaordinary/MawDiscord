...
class CharacterRequest(Request):
    ...
    def handle(self, engine, tokenizer, discord_loop, channel_queue):
        self.history.append_message(Message(self.message.id, self.prompt, "user"))
...
class ScrollRequest(CharacterRequest):
    def __init__(self, message, bot_message, history, prompt, cutoff, tools, edit, view, idxs):
        super().__init__(message, bot_message, history, prompt, cutoff, tools, edit)
        self.view = view
        self.idxs = idxs
    def handle(self, engine, tokenizer, discord_loop, channel_queue):
        history = self.history.to_tokenizer(limit=self.message.id)
